...
savePresetBtn.addEventListener("click", () => {
    ...
    const presets = getPresets();
    presets[name] = kernelInput.value;
    setPresets(presets);
    updatePresetList();
});
...
function updateFPS() {
    ...
    if (elapsed >= fpsInterval) {
        ...
        // 1% Low FPS (worst 1% frame times → lowest 1% FPS)
        const sorted = [...frameTimes].sort((a, b) => b - a); // longest frame times first
        const onePercentIndex = Math.floor(sorted.length * 0.01);
        const onePercentLow = sorted[onePercentIndex] || sorted[sorted.length - 1];
        const onePercentFps = Math.round(1000 / onePercentLow);

        fpsCounter.textContent = `FPS: ${fps} | Avg: ${avgFps} | 1% Low: ${onePercentFps}`;
